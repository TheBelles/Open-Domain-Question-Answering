...
def apply_rotary_emb(x, position_embeddings):
    ...
    sin = sin.unsqueeze(1)  # (B, 1, seq_len, head_dim // 2)
    x_even = x[..., ::2]
    x_odd = x[..., 1::2]
    ...
    x = torch.stack([x_even_rot, x_odd_rot], dim = -1).flatten(-2)
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        #### YOUR CODES; TODO 
        rms_square = torch.mean(torch.square(x), dim=-1, keepdim=True)
        rms = torch.sqrt(rms_square + self.eps)
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        with torch.autocast(device_type=x.device.type, enabled=False): 
            ...
            #### YOUR CODES; TODO 
            theta = position_ids_expanded * inv_freq_expanded
            cos = torch.cos(theta).transpose(-2, -1)  # (B, head_dim // 2, seq_len)
            sin = torch.sin(theta).transpose(-2, -1)  # (B, head_dim // 2, seq_len)
            ######
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        attn_output_weights = F.softmax(attn_weights, dim=-1)

        attn_output_weights = F.dropout(attn_output_weights, self.dropout, training=self.training)
        attn_output = torch.matmul(attn_output_weights, value_states)
...
class FeedForwardNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        x_hidden = self.dropout(x_up * x_gate)
        outputs = self.down_proj(x_hidden)
        ######
        return outputs
...
class TransformerLayer(nn.Module):
    ...
    def forward(
        ...
        attn_output, past_key_value = self.self_attn(normed, position_embeddings, attention_mask, past_key_value)
...
class TransformerModel(TransformerPreTrainedModel):
    ...
    def _prepare_attention_mask(
        ...
        #### YOUR CODES; TODO 
        causal_mask = torch.tril(torch.ones((sequence_length, target_length), dtype=torch.bool, device=device), diagonal=seen_token_length)
        causal_mask = causal_mask.unsqueeze(0).unsqueeze(0)  # (1, 1, sequence_length, target_length)

        if attention_mask is not None:
            attention_mask = attention_mask.unsqueeze(1).unsqueeze(1)
            attention_mask = attention_mask.expand(batch_size, 1, sequence_length, target_length)

            combined_mask = attention_mask.bool() & causal_mask
        else: 
            combined_mask = causal_mask.expand(batch_size, 1, sequence_length, target_length)

        # Convert to attention weights format: 0.0 for positions to attend to, -inf for positions to mask
        mask = torch.zeros_like(combined_mask, dtype=dtype)
        mask = mask.masked_fill(~combined_mask, torch.finfo(dtype).min)  
